...
const CustomRule = () => {
  return {
    ...
    // 3. [\u4e00-\u9fa5a-zA-Z0-9\s\p{P}\p{S}] -> 允许的字符范围：汉字、字母、数字、空白字符(\s)、标点符号(\p{P})和特殊符号(\p{S})
    // 4. {1,50} -> 限制总长度为1到50个字符
    pattern: /^(?!^[\u4e00-\u9fa5]+$)[\u4e00-\u9fa5a-zA-Z0-9\s\p{P}\p{S}]{1,50}$/u,
    message: "证件号码格式不正确（长度1-50，允许汉字/字母/数字及常见符号）",
    trigger: "blur"
  };
};
