...
const tableconfig = reactive<AppTableConfig>(
	createTableEditConfig({
        ...
        createFreeButtonBase({
            ...
            icon: "Edit",
            tableClick: (row) => {
            dzmodal.open(chatsMgrEdit, { type: "update", data: row }).then((res) => {                
                if (res.type === "ok") {
                handleQuery();
                }
            });
        ...
        createFreeButtonBase({
            ...
            tableClick: (row) => {
              ...
              ElMessageBox.confirm('确认要删除吗？该数据删除之后将无法恢复。', '提示', {
                ...
                sysRolechatsMgrService.delRoleChatsById({ cPkId: row.cPkId }).then((res: any) => {
                  if (res.code === 200) {
...
function handleQuery(flag?: boolean) {
	...
	sysRolechatsMgrService.getRoleChatsList(param)
		.then((res) => {
