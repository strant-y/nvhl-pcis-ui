...
import { savePrdTermInfo,queryPageComponentList,initProdEdrRsnItemList,saveProdEdrRsnItemList } from "@/api/prod"; // api接口
import {
    AppTableConfig,
    ...
    createFreeButtonBase({
        ...
        func: async () => {
            ...
            sel['selectFactor'].forEach((v)=>{
                const obj={}
                obj['CGroup']=v.cGroup
                obj['CFactorKey']=v['factorPkId']
                obj['CEdrItem']=v['c_factor_tab']
                obj['COperTyp']='M'
...
function selectComponent(item: any) {
    ...
    const param={CComponentKey:item.cComponentKey,cComponentTab:item.cComponentTab ,CProdNo:item['c_prod_no']}
...
onMounted(() => {
    queryPageComponentList({pageSelect:{cProdNo:props.data.cProdNo}})
        .then((res) => {
            const { code, data, msg } = res;
            if (200 === code) {
