...
import {
  AppTableConfig, createTableEditConfig,
  MyTableMethod,
} from "@/shared/app-table-config";
import { useDzModal } from "@/common/dzmodel/DzModalService";
import { eventBus } from '@/utils/event-bus'
import {idxParamKey, IdxParamProps, useIdxParam} from "@/views/pcis/support/useIdxParam";
import {CustomStructure, CompositePageView, CommonGroupId} from "@/views/pcis/support/composite.types";
...
const refreshData = () => {
  ...
  const cCombinationPlanNo = param.cCombinationPlanNo;
  ...
  const getResult = (result: any[]) => {
    ...
    if (parparam.pageType  == "app" && !parparam.cCombinationNo) {
      ...
      setFormValue([...sel], CommonGroupId)
    }else{
      originalData.value.push(...deepClone(result))
    }
    console.log('getpSpecialAgreement-formData.value', formData.value)
  }
  // 查询列表数据
  const reqParam: any = {
    cDptCde: cDptCde,
...
function setFormValue(value: any, groupId: string) {
  if(groupId !== CommonGroupId) {
    return;
  }
  if(value && value.length>0) {
    const list = value.map((item: any, idx: number) => {
      const e: any = {}
...
onMounted(async () => {
  const pageSchema = structure.mergeComps(props.pageSchemaList)
  console.log('特约信息-pageSchema', pageSchema)
