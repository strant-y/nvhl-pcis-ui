...
import {CustomStructure} from "@/views/pcis/support/composite.types";
...
function getFromValue(groupId?: string) {
  const formData = tgtEditRef?.value?.getFromValue();
  const resultData: any = {}
  const prodNo = groupId ? groupId.replace('group-', '') : undefined;
  formconfig1.fromSchema?.filter((f: any) => !prodNo || prodNo === '000000' || (f.prodKeys && f.prodKeys.includes(prodNo)))
      .forEach((item: any) => {
        if(item.mergedType === '2') {
          resultData[item.prop.split(":")[1]] = formData[item.prop]
        } else {
          resultData[item.prop] = formData[item.prop]
        }
      })
  return resultData
}
...
function setFormValue(value: any, groupId?: string) {
  ...
  formconfig1.fromSchema?.filter((f: any) => !prodNo || prodNo === '000000' || (f.prodKeys && f.prodKeys.includes(prodNo)))
      .forEach((item: any) => {
        if(item.mergedType === '2') {
          resultData[item.prop] = value[item.prop.split(":")[1]]
        } else {
          resultData[item.prop] = value[item.prop]
        }
      })
  tgtEditRef?.value?.setFormValue(resultData);
}
